...
Hudson^f^, Emma M. Karlsson^b^, Mairéad MacSweeney^e^, Heather Payne^e^,
...
Dräger, Bobe, Lohmann et al., 2000; Rasmussen & Milner, 1976, 1977;
...
Bethmann, Tempelmann, De Bleser, Scheich, & Brechmann, 2007, Sørensen &
...
fMRI (Bethmann et al., 2007; although see Sørenson & Westerhausen, 2020,
...
● Aged 16-50 years. The younger age limit avoids developmental change in
...
● Access to a laptop or desktop computer with stereo headphones for use
...
● Unwillingness to travel to one of the testing sites for Step 2 of the
study\
● Contraindications or unwillingness to participate in fMRI in future
parts of the study.

`r suppnum<-suppnum+1`

The initial screening questionnaire was administered on the Gorilla
platform (www.gorilla.sc; Anwyl-Irvine, Massonnié, Flitton, Kirkham &
...
`r suppnum <- suppnum+1` \### 2.3.1 Demographics Questionnaire\
...
`r suppnum<-suppnum+1` \### 2.3.2 Edinburgh Handedness Inventory (EHI)
...
Lemhöfer & Broersma, those scoring below 80 are not eligible for
...
`r suppnum <- suppnum+1` \#### 2.3.6.1 Materials\
...
`r suppnum <- suppnum+1` \#### 2.3.7.1 Materials\
A total of 108 experimental images were selected from the MultiPic
databank (Duñabeitia et al., 2018). As Duñabeitia et al. had
...
(Duñabeitia et al., 2018), in order to create stimuli for three new
trials.
...
using the interval from −10 to 0 s pre-stimulus time (where the onset of
...
`r fignum<-fignum+1` \### Figure `r fignum`\
...
the RVAideMemoire package (version 0.9-79; Hervé, 2021) with 1,000
...
correlations between LIs for the different tasks as a heatmap. The
...
of measures. The heatmap shows moderate correlations within both
...
between the two, even for the two measures, Word Decision and Rhyme
...
![Heatmap with behavioural and fTCD laterality
indices](figs/bigheatmap.eps)
...
should also have seen this bias in Rhyme Decision, and we should not
have seen it in the fTCD Word Decision task, where pictures are
...
inhibitory demands of the task (Jäncke & Shah, 2002; Westerhausen et
al., 2014).
...
Jäncke, L., & Shah, N. J. (2002). Does dichotic listening probe temporal
...
Knecht, S., Deppe, M., Dräger, B., Bobe, L., Lohmann, H., Ringelstein,
...
Ocklenburg, S., Ströckens, F., Bless, J. J., Hugdahl, K., Westerhausen,
...
cerebral speech areas in man. In K. Zülch, O. Creutzfeldt, & G.
...
Sørensen, Ø., & Westerhausen, R. (2020). From observed laterality to
...
Conceptualisation, Methodology, Writing - Review and Editing; Mairéad
