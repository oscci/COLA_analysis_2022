...
lab2<- paste0("Group 1: rho = ",round(cor2$estimate,3),": 95% CI: ",round(cor2$conf.int[1],3),' to ',round(cor2$conf.int[2],3))
...
## 2.2 Procedure
There were two stages to the project. All participants who proceeded beyond the initial screening  completed the online testing (Step 1), and a subset of participants were invited back for the fTCD session (Step 2). Participants received course credit or were paid in accordance with guidelines at their local testing centre (at least £8 per hour).  
...
This task was based on Mazoyer et al. (2014). Participants were shown a line drawing in each trial and asked to produce a meaningful sentence to describe it. The 18 stimuli were selected from those used by Woodhead et al. (2018).  
...
_The pre-registration document stated that "Epochs are baseline corrected using the interval from −10 to 0 s pre-stimulus time (where the onset of the ‘Clear’ stimulus is used as the start of the trial, 0 s pre-stimulus time)."  Woodhead et al used a shorter interval of -5 to 2 seconds for baseline correction to avoid activity from the prior trial influencing the baseline. We had aimed to avoid that problem by having slightly longer epochs, but inspection of blood flow plots (see Figure `r fignumber+4` below) showed this was not the case, and so we reverted to the baseline of -5 to 2 s. This avoids unstable baselines, and is consistent with the baseline period used by Woodhead et al. For completeness, the script to create this paper has been rerun using the preregistered baseline, and is shown in Supplement x. This indicates that the shorter baseline gives slightly cleaner data, but does not exert any material impact on the main findings._  
...
To justify this approach, we first considered whether either raw or absolute laterality indices were correlated with either of the language screening measures and showed they were not  (See Supplementary file `r suppnum<-suppnum+1` `r suppnum`. `r suppcorr<-suppnum`).
...
Figure `r fignumber` divides participants according to the language screen. Group H (high proficiency) corresponds to cases who meet preregistered criteria: they are either native English speakers, or non-native speakers who passed the preregistered criterion for language competence (LexTALE of 80 or more, and no more than 3 errors on full Games with Words test). Group M (moderate proficiency) is subdivided into subgroup (a), who failed the original Games with Words criterion, but passed the revised criterion, and subgroup (b), who failed the screen using the new, shorter test (making more than 1 error). Here we report data on the whole sample with no exclusions based on the language screen, but in Supplementary file 2 we compare our results with those obtained using the original, stringent language cutoff (excluding all Group M), and show that differences are minimal.
...
In our pre-registration we stated that we would compute a conventional laterality index, 100 x (Left-Right)/(Left+Right), as well as a laterality z-score:
...
Table `r tabnumber`: _Descriptive statistics for LIz for three online laterality measures_  
...
## 3.5 Exploratory Analysis of Relationship between Behavioural Performance and Laterality Indices
...
The finding of a slight bias to right hemisphere lateralisation for single word comprehension  is puzzling. This is especially the case given that it was found both in the online behavioural version of the task (see Table `r tabonlinesummary` and in the analogous Word Decision fTCD task (see Table `r ftcdtabnumber`). Furthermore, on the behavioural task, left-handers showed a stronger effect than right-handers. This consistent picture, however, is challenged by the fact that the correlation between LIs on the online and fTCD versions of the task was close to zero.   
...
Sentence Decision also behaved unexpectedly, in that it had significant loadings on Factor 1, despite being designed as a purely receptive task. While it is possible that participants might have covertly repeated sentences to themselves when doing the task, the fast pace of the task made that unlikely, and furthermore, our previous study suggested that simple production of spoken output was not the key attribute of Factor 1 tasks; we had dropped from the current study a List Generation task used by Woodhead, Rutherford and Bishop (2020) that simply involved repeating overlearned sequences such as days of the week, as it had been only weakly lateralised. The Sentence Decision task did, however, involve  using syntactic information to assign semantic roles and build meaning representations, and hence more linguistic computation than the tasks that used single word stimuli. 
...
SAY SOMETHING ABOUT HOW CONCLUSIONS MESH WITH WOODHEAD ET AL.
