...
mysummary[11,writecolnum]<-paste0(round(cor.eo$estimate,3),' [',round(cor.eo$conf.int[1],3),', ',round(cor.eo$conf.int[2],3),']')
...
RDTsides <- ggplot(RDTdat, aes(x=exp(RDT.Lmean), y=exp(RDT.Rmean), color=Handedness,shape=as.factor(RDTsig))) +
...
WCsides <- ggplot(WCdat, aes(x=exp(WC.Lmean), y=exp(WC.Rmean), color=Handedness,shape=as.factor(WCsig))) +
  xlab("Mean correct RT (ms) left VHF")+
   ylab("Mean correct RT (ms) right VHF")+
  geom_point()+
    xlim(500,1600)+
...
onlinesummary[12,3]<-paste0(round(cor3$estimate,3),'[',round(cor3$conf.int[1],3),', ',round(cor3$conf.int[2],3),']')
...
onlinesummary[12,4]<-paste0(round(cor4$estimate,3),' [',round(cor4$conf.int[1],3),', ',round(cor4$conf.int[2],3),']')
...
pirateplot(data = longdata.b, LIz ~ Handed * Task)
abline(h=0)
