...
dev.off() #should suppress dev.off in text
```
...
#More sense to make changes to model based on observation of heatmap.
#Low correlations between D and F & Fac1 and path A and B & Fac2. We can drop these
...
It is noteworthy that this more categorical analysis finds rates of "atypical", i.e. non-left, lateralisation on language tasks that are on the one hand task-dependent, but on the other hand much lower than typically observed when methods such as Wada test or fMRI are used. This is the case even for the most lateralised task, Sentence Generation, where 91% of right-handers vs 77% of left-handers were left-lateralised. 
...
model.2Fnx <- '
f1 =~  A_mean_LI+B_mean_LI+C_mean_LI+E_mean_LI+RDT.zlat.ex
f2 =~  F_mean_LI+D_mean_LI+E_mean_LI+C_mean_LI+DL.zlat.ex
'
fit.2Fnx <- cfa(model.2Fnx,estimator="WLSMV", data=mycomb)
summary(fit.2Fnx)
wantfits <- c('CFI','rmsea','chisq','df')
fitmeasures(fit.2Fnx,wantfits)
measurementInvariance(model=model.2Fnx,estimator="WLSMV",eanstructure=T,data=mycomb,group="Handedness")


model.2Fny <- '
f1 =~  A_mean_LI+B_mean_LI+C_mean_LI+E_mean_LI+RDT.zlat.ex+DL.zlat.ex
f2 =~  F_mean_LI+D_mean_LI+E_mean_LI+C_mean_LI
'
fit.2Fny <- cfa(model.2Fny, estimator="WLSMV",data=mycomb)
summary(fit.2Fny)
wantfits <- c('CFI','rmsea','chisq','df')
fitmeasures(fit.2Fny,wantfits)
measurementInvariance(model=model.2Fny,estimator="WLSMV",data=mycomb,eanstructure=T,group="Handedness")
...
model.2Fnz <- '
f1 =~  A_mean_LI+B_mean_LI+C_mean_LI+E_mean_LI+DL.zlat.ex
f2 =~  F_mean_LI+D_mean_LI+E_mean_LI+C_mean_LI
'
fit.2Fnz <- cfa(model.2Fnz, estimator="WLSMV",data=mycomb)
summary(fit.2Fnz)
wantfits <- c('CFI','rmsea','chisq','df')
fitmeasures(fit.2Fnz,wantfits)
measurementInvariance(model=model.2Fnz,estimator="WLSMV",data=mycomb,meanstructure=T,group="Handedness")

model.2Fnzz <- '
f1 =~  A_mean_LI+B_mean_LI+C_mean_LI+DL.zlat.ex
f2 =~  F_mean_LI+D_mean_LI+E_mean_LI+C_mean_LI
'
fit.2Fnzz <- cfa(model.2Fnzz, estimator="WLSMV",meanstructure=T,data=mycomb)
summary(fit.2Fnzz)
wantfits <- c('CFI','rmsea','chisq','df')
fitmeasures(fit.2Fnzz,wantfits)
measurementInvariance(model=model.2Fnzz,estimator="WLSMV",data=mycomb,group="Handedness")

model.2Fnzzz <- '
f1 =~  A_mean_LI+B_mean_LI+C_mean_LI
f2 =~  F_mean_LI+D_mean_LI+E_mean_LI+C_mean_LI+DL.zlat.ex
'
fit.2Fnzzz <- cfa(model.2Fnzzz, estimator="WLSMV",data=mycomb)
summary(fit.2Fnzzz)
wantfits <- c('CFI','rmsea','chisq','df')
fitmeasures(fit.2Fnzzz,wantfits)
measurementInvariance(model=model.2Fnzzz,estimator="WLSMV",data=mycomb,meanstructure=T,group="Handedness")
