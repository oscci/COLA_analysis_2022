...
Our original flowchart had stated there were four online laterality tasks, but, as stated in the Stage 1 report, one of these had been dropped after pilot testing showed it had poor reliability, and only three tasks were therefore used. The original flowchart from the Stage 1 report was in error in mentioning 4 tasks, and this is corrected here.  
...
Table `r tabnumber` shows demographic data for the subset of individuals tested on the online battery only, and the subset who also completed the session with fTCD. It is evident from inspection that differences between the two subgroups are not substantial. 
...
The mean number of words produced per trials was `r round(acctab$mean[1],2`, (SD = `r round(acctab$sd[1],2`) for Word Generation and  `r round(acctab$mean[2],2`, (SD = `r round(acctab$sd[2],2`) for Sentence Generation. For the four decision tasks, accuracy was recorded in percentage correct:  `r round(acctab$mean[3],2`, (SD = `r round(acctab$sd[3],2`) for Phonological Decision, `r round(acctab$mean[4],2`, (SD = `r round(acctab$sd[4],2`) for Word Decision, `r round(acctab$mean[5],2`, (SD = `r round(acctab$sd[5],2`) for Sentence Decision, and `r round(acctab$mean[6],2`, (SD = `r round(acctab$sd[6],2`) for Syntactic Decision.  
...
![Distributions of fTCD LIs on six tasks for 104 left-handers and 91 right-handers. Tasks ordered in strength of left-lateralisation, from most to least.](ftcd_pirates.jpg)
...
![One-factor vs. two-factor structural equation models](path_diags_asppt.jpg)
...
semPaths(fit1, "diagram", weighted = FALSE,  shapeMan = "rectangle", sizeMan = 12, 
    sizeMan2 = 5,nodeLabels=mynodes, filetype='tiff',filename=pathfigname,width=3,height=1.8) #draws a path diagram
...
semPaths(fit2, "diagram", weighted = FALSE,  shapeMan = "rectangle", sizeMan = 12, 
    sizeMan2 = 5,nodeLabels=mynodes2,filetype='tiff',filename=pathfigname,width=3,height=1.8) #draws a path diagram
...
cohd1 <- cohen.d(ddati$Factor1[ddati$Handed=="Right"],ddati$Factor1[ddati$Handed=="Left"])
cohd2 <- cohen.d(ddati$Factor2[ddati$Handed=="Right"],ddati$Factor2[ddati$Handed=="Left"])
