...
__We may not stick to this; I will press on for now assuming this. We have been asked to demonstrate specific changes to exclusion rules don't make a big difference to results__
...
_I've put the plots at the end in Supplementary materials, to keep the paper short, but it doesn't work all that well, as you need to see the plot to understand the commentary. Interested in advice on how to best present this._

_ZW: Can those 5 plots be combined into one multi-panel figure in the main MS? Or, put the descriptions and figures 1-3 in supplementary materials, and go straight on to the Laterality Indices section (and figs 4 and 5) in the main MS?_
...
As is evident from Supplementary figure 4, the z-LI score is very highly correlated with the conventional LI, the principal difference being that the z-LI follows the normal distribution, with a sigmoid shape at the extremes (truncated in the Figure because of the censored scale). For subsequent analyses, we use z-LI, as this allows us to compare different tasks on a common scale.  

__2-3. Rhyme Decision and Word Comprehension__  
Density plots and scattergrams for these two tasks are shown together in Supplementary Figure 5, as both have z-LIs computed on the basis of correct RTs to visual stimuli presented to the left and right.  
...
pcut <- .001 #found normalisation by BoxCox created abnormally large variance for variable E - better work with non-normalised. Achieve that by just setting pcut very extreme, so no variable meets criterion for nonnormality
...
#Look at effect of either excluding data or censoring on normality.

#we'll skip this as although it helped iwth normalisation it did not fix problems with SEM fit
doskip<-1
if(doskip==1){
...
for (i in 1:6){
  cutoff<-mean(thisdat[,i],na.rm=T)+4.5
  replace<-NA
  w1<-which(thisdat[,i]>cutoff)
    w2<-which(thisdat[,i]<(-cutoff))
    print(length(w1))
    if(length(w1)>0){
    thisdat[w1,i]<-replace
    }
    print(length(w2))
    if(length(w2)>0){
      thisdat[w2,i]<-(-replace)
    }
}


  mvn(thisdat,mvnTest = c("mardia", "hz", "royston", "dh",
  "energy")) #useful for getting normality check on all vars

  #Censoring using absolute cutoff values of 4 or 5 makes it worse
  #But removing extreme values does better
  
colnames(thisdat)<-nunames
}
```
...
Figure x shows scatterplots of the bivariate relationships between the three variables. It is evident from inspection that we can reject model C, in which all three LIs are independent, and model B1, where only Dichotic Listening and Rhyme Decision are correlated. The strongest correlation is between the two visual tasks, Rhyme Decision and Word Comprehension, as predicted by model B2. 

Note that correlations will be influenced by test reliability. Indeed, the correlation between Rhyme Decision and Word Comprehension is close in magnitude to the split half reliability of the two measures. 
...
Structural equation modeling (Kline, 2011) is a method that allows a formal test of adequacy of competing models for explaining patterns of association between variables. The underlying assumption of this approach is that observed variables can be treated as indicators of underlying, unobserved latent variables. Figure 1x shows the single factor model on the left, and a two-factor model on the right. The latent factors are shown in ovals, and the observed variables in boxes. Single-headed arrows indicate causal paths, and double-headed arrows indicate variances. Although means can be incorporated in SEM (and we shall be doing this in our analysis), the main use of SEM is to analyse patterns of covariances. The important point to note is that the path diagrams shown in Figure 1x have a precise mathematical interpretation, and can be converted into linear equations that specify the covariances between observed variables. Thus it is possible to obtain a measure of goodness of fit for observed data in relation to a model by comparing whether the observed covariances agree with those predicted by the model. We can already see by inspecting the heatmap of Figure x that a single-factor model is unlikely to provide a good fit to the observed data, because it would not predict similar the clustering of correlations that is evident. 
...
![Single factor](~/Dropbox/COLA_RR_Analysis/03-graphic-outputs/PathfigF1.jpg) 
![Two factor](~/Dropbox/COLA_RR_Analysis/03-graphic-outputs/PathfigF2.jpg) 
...
```{r factormodels,echo=F,include=F}
...
```
We used the lavaan() package to perform the pre-registered model comparison. Table z summarises the main output of the model-fitting. The fit of both the one-factor and the two-factor model is poor. 
...
model.2Fall <- '
f1 =~  NA*A_P1 + B_P2+C_P3+D_R1+ E_R2
f2 =~  NA*B_P2+C_P3+D_R1+ E_R2 +F_R3
f1~~1*f1
f2~~1*f2
'
fit.2Fall.1 <- cfa(model.2Fall, data=mydat)
...
#More sense to make changes to model based on observation of size of paths.
#Path D to Fac1 and path B to Fac2 are NS. We can drop these

#After checking model from group 1, now try running with 2nd group and with full sample

model.2Fn <- '
f1 =~  NA*A_P1 + B_P2+C_P3+ E_R2
f2 =~  NA*F_R3 + C_P3+D_R1+ E_R2   #2 factor model: no constraint on covariance
f1~~1*f1
f2~~1*f2'
...
try3<-0
if(try3==1){
#try a 3 factor version? Highest residuals are between A, E F, so what is it like with extra factor just for those 3?
model.3Fn <-" f1 =~  NA*A_P1 + B_P2+C_P3+D_R1
...
"
fit.3Fn.1 <- cfa(model.3Fn, data=mydat)
tab.3Fn.1 <- makeSEMtab(fit.3Fn.1)
...
f1~~1*f1
f2~~1*f2
#Option in model: adding constraints to force obs variable variances to be > .01
# A_P1 ~~ varA*A_P1
# B_P2 ~~ varB*B_P2
# C_P3 ~~ varC*C_P3
# D_R1 ~~ varD*D_R1
...
fitM0 <- cfa(model.2Fn, data = ddati,meanstructure=T)
tabM0 <- makeSEMtab(fitM0)  #

# configural invariance
fitM1 <- cfa(model.2Fn, data = ddati,meanstructure=T, group = "Handed")

tabM1 <- makeSEMtab2(fitM1,c('_L','_R'))  #in tab1, everything varies for the 2 groups

# weak invariance
fitM2 <- cfa(model.2Fn, ddati, meanstructure=T,group = "Handed",
...
# strong invariance
fitM3 <- cfa(model.2Fn, ddati, meanstructure=T,group = "Handed",
...
fitM3a <- cfa(model.2Fn, ddati,meanstructure=T, group = "Handed",
...
fitM4 <- cfa(model.2Fn, ddati, meanstructure=T,group = "Handed",
 group.equal = c("loadings","intercepts","means","lv.covariances"))

tabM4 <- makeSEMtab2(fitM4,c('_L','_R'))

# model comparison tests
lavTestLRT(fitM0,fitM1, fitM2, fitM3,fitM3a, fitM4)
...
```{r LRmodel1,echo=F,include=F}
#Seem worth also testing for equivalence with single factor model

#This is definition of single factor model we will use here. Makes it easier to compare loadings if f1 variance = 1, rather than fixed path - gives same fit as previous but easier to interpret.

model.1Fh <- 'f1 =~  NA*A_P1 + B_P2  + C_P3 +D_R1 + E_R2 + F_R3
f1~~1*f1' 


fit1L <- cfa(model.1Fh, data=ddati[ddati$Handed=='Left',]) #runs the model and saves results in fit1
sfit1L <- makeSEMtab(fit1L)
fit1R <- cfa(model.1Fh, data=ddati[ddati$Handed=='Right',]) #runs the model and saves results in fit1
sfit1R <- makeSEMtab(fit1R)
#Confirms that fit is poor for both L and R handers; in both groups, paths to all the indicators are significant.

measurementInvariance(model=model.1Fh,data=ddati,meanstructure=T,group="Handed")
#For this model the intercepts are significantly different for the 2 handedness groups. However, since the model fit is so poor, not sure this is meaningful. 
```

So results from measurement invariance test show that model fit does not worsen when intercepts are fixed, but the factor means for the two groups are very different.
...
After testing associations for individual measures, we categorise individuals as either consistently left-lateralised on all tests, or right-lateralised on one or more tests, and conduct a chi-square test contrasting the proportion of left- and right-handers on this composite measure.
...
tabN4a
round(prop.table(tabN4a,2),3)

```
# Are online behavioural measures comparable to direct measures of cerebral blood flow in indexing language laterality?

A preliminary inspection of correlations between online and fTCD laterality indices showed very little relationshp between the two, even for the two measures, Word Comprehension and Rhyme Decision, that has analogues in fTCD (tasks D and C respectively).

```{r bigheatmap,echo=F}

mydf <- combdat[combdat$DopExclude==0,] #need to ensure online LIs from excluded are not included
mycolsb<-c("DL.zlat","RDT.zlat","WC.zlat",  "A_mean_LI", "B_mean_LI","C_mean_LI", "D_mean_LI","E_mean_LI", "F_mean_LI")
bigheatmap <- makeheatmap(mydf,mycolsb)
bigheatmap
...
model.2Fnx <- '
f1 =~  A_mean_LI+B_mean_LI+C_mean_LI+E_mean_LI+RDT.zlat
f2 =~  F_mean_LI+D_mean_LI+E_mean_LI+C_mean_LI+DL.zlat
'
fit.2Fnx <- cfa(model.2Fnx, data=mycomb)
summary(fit.2Fnx)
wantfits <- c('CFI','rmsea','chisq','df')
...
model.2Fny <- '
f1 =~  A_mean_LI+B_mean_LI+C_mean_LI+E_mean_LI+RDT.zlat+DL.zlat
f2 =~  F_mean_LI+D_mean_LI+E_mean_LI+C_mean_LI
'
fit.2Fny <- cfa(model.2Fny, data=mycomb)
summary(fit.2Fny)
wantfits <- c('CFI','rmsea','chisq','df')
...
#Just for completeness - check that RDT does not work with F2
model.2Fnz <- '
f1 =~  A_mean_LI+B_mean_LI+C_mean_LI+E_mean_LI+DL.zlat
f2 =~  F_mean_LI+D_mean_LI+E_mean_LI+C_mean_LI+RDT.zlat
'
fit.2Fnz <- cfa(model.2Fnz, data=mycomb)
summary(fit.2Fnz)
wantfits <- c('CFI','rmsea','chisq','df')

fitmeasures(fit.2Fnz,wantfits)
tab.2Fnz <- makeSEMtab(fit.2Fnz)
#Yes - the path from RDT to F2 is NS


```
The pattern of correlations shown in the heatmap already suggests that the pre-registered model, with Rhyme Judgement loading on Factor 1, and Dichotic Listening on Factor 2, will not be a good fit, because Dichotic Listening is correlated more strongly with the fTCD measures that load on Factor 1. This was confirmed: the path from Dichotic Listening to Factor 2 was non-significant.  

Guided by the data, we ran an alternative model (not pre-registered) with both Rhyme Judgement and Dichotic Listening loading on Factor 1. This gave a better fit, with significant paths from both of these measures, as shown in Figure x. 

```{r modelfitbighand,echo=F}
#Redo best fitting model with handedness groups
measurementInvariance(model=model.2Fny,data=mycomb,meanstructure=T,group="Rhanded")
...
Previous attempts to consider the dimensionality of language lateralisation have been obscured by two issues. First, researchers have tended to focus only on measures that show left-lateralisation at the population level, treating unlateralised measures as uninteresting. Second, many studies have been conducted with measures whose reliability was not established. If two laterality indices are not correlated, it could just be because they are unreliable, and so it has been easy to dismiss lack of correlation between laterality measures as uninformative. 

Considering first the online behavioural data, the most noteworthy observation was that the correlations between laterality indices from the three tasks were generally weak. This could not be attributed solely to poor reliability: although reliabilities of the two new tasks, Rhyme Decision and Word Comprehension, were not impressive (ranging from .54-.55 for test-retest), they were higher than the intercorrelations between measures. Our data was not suitable for a more formal test, but inspection of the pattern of correlations between measures made it clear that our proposed two-factor model, with Dichotic Listening and Word Comprehension being positively correlated and unrelated to Rhyme Decision could not be supported. Indeed, the strongest correlation was found between Word Comprehension and Rhyme Judgement, consistent with the idea that task demands (speeded responding to visual pictures) might be a greater determinant of strength of lateralisation than whether receptive or expressive language was involved.  

For the fTCD data, we were able to subject the single factor model to a stronger test, because we had sufficient tasks for Structural Equation Modeling. In addition, and consistent with Woodhead et al (xxxx), we confirmed here that tasks that are not lateralised at the population-level can nevertheless have highly reliable laterality indices, indicating that individual differences in degree of lateralisation are stable.

Our use of structural equation modeling leads to a focus on correlations between laterality indices while largely disregarding means. This approach reveals that two language tasks can have correlated laterality indices, even when one task is lateralised at the population level and the other is not - for example Phonological Decision and Word Comprehension. Such observations seem compatible with a simple model, in which a person's strength of lateralisation is consistent across language tasks, but tasks vary in terms of their mean laterality bias. 

Our previous study using SEM found that this simple model could not adequately account for the pattern of correlations between laterality indices. Rather, the correlations patterned in a way that suggested there were (at least) two underlying factors that influenced language lateralisation. The current study again failed to support a single factor model of lateralisation.  The alternative model that we had proposed, that involved a division between language production and language reception accounted for more variance, but still left a great deal unexplained, and overall the fit was poor. Accordingly, we divided the sample into two subgroups to explore different models and found one that gave reasonable fit, which was then replicated in the second half of the sample. This again had a two factor structure  


## Supplementary materials  

Supplementary Figure 1
![Scatterplot showing items correct from left and right ear on Dichotic Listening task.](~/Dropbox/COLA_RR_Analysis/03-graphic-outputs/Afig1_DLsides.png)


Supplementary Figure 2
![Scatterplot showing items correct from left and right ear on Dichotic Listening task.](~/Dropbox/COLA_RR_Analysis/03-graphic-outputs/Afig2_RDTsides.png)

Supplementary Figure 3
![Scatterplot showing items correct from left and right ear on Word Comprehension task.](~/Dropbox/COLA_RR_Analysis/03-graphic-outputs/Afig3_WCsides.png)
