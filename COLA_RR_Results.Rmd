...
Table `r tabnumber`: _Descriptive statistics for LIz for three online laterality measures (LIz)_  
...
pathfigname1<-paste0(mydir,"/",figdir,"/pathfigF1")
mynodes<-c('Word\nGeneration','Sentence\nGeneration','Phonological\nDecision','Word\nDecision','Sentence\nDecision','Syntactic\nDecision','F1')

semPaths(fit1, "diagram", weighted = FALSE,  shapeMan = "rectangle", sizeMan = 12, 
    sizeMan2 = 5,rotation=4,edge.color='black',nodeLabels=mynodes, filetype='eps',filename=pathfigname1,width=8,height=6) #draws a path diagram
...
#lavResiduals(fit2)
pathfigname2<-paste0(mydir,"/",figdir,"/pathfigF2")
mynodes2<-c(mynodes,'F2')
semPaths(fit2, "diagram", weighted = FALSE,  shapeMan = "rectangle", sizeMan = 12, 
    sizeMan2 = 5,rotation=4,edge.color='black',nodeLabels=mynodes2,filetype='eps',filename=pathfigname2,width=8,height=6) #draws a path diagram
...
imageX<-image_read(paste0(pathfigname1,'.eps'),density=300)
imageY<-image_read(paste0(pathfigname2,'.eps'),density=300)
...
mynodes3<-c('Word\nGeneration','Sentence\nGeneration','Phonolog.\nDecision','Sentence\nCompr.','Word\nCompr.','Syntactic\nDecision','F1','F2')
semPaths(fit.2Fn, "diagram", weighted = FALSE,  shapeMan = "rectangle", sizeMan = 12, 
    sizeMan2 = 5,nodeLabels=mynodes3,rotation=2,filetype='eps',filename=pathfigname,width=3,height=1.8) #draws a path diagram

#redo with save to current directory
pathfigname<-"pathfigF2_revised"
semPaths(fit.2Fn, "diagram", weighted = FALSE,  shapeMan = "rectangle", sizeMan = 12, 
    sizeMan2 = 5,nodeLabels=mynodes3,filetype='eps',filename=pathfigname,width=3,height=1.8) #draws a path diagram
...
Table `r tabnumber` summarises the characteristics of the six fTCD tasks, grouped according to the factors they load on. The online Dichotic Listening task is also shown. The task battery had been designed to include three tasks that involved language generation, and three that involved receptive language. As predicted, the former loaded on Factor 1 and the latter on Factor 2, but in addition Phonological Decision loaded on Factor 2, and Sentence Decision on Factor 1. Phonological Decision, unlike the other tasks loading on Factor 2, did not involve auditory input, but did have in common the 2-choice response format of other Factor 2 tasks. 

Sentence Decision also behaved unexpectedly, in that it had significant loadings on Factor 1, despite being designed as a purely receptive task.  Unlike the other two receptive tasks, Sentence Decision involved using syntactic information to assign semantic roles and build meaning representations, and hence more linguistic computation than the tasks that used single word stimuli. 
