...
```{r checkviewport, echo=F}
#When doing data dictionary, noted some extreme values for viewport.
#Should be excluded.
...
for (i in 1:nrow(allsum)){
  id <- allsum$ID[i]
  w<-which(all_dat$Participant.Private.ID==id)[1]
  myviewport<-all_dat$Participant.Viewport.Size[w]
  allsum$viewport1[i]<-as.numeric(unlist(strsplit(myviewport,'x'))[1])
 allsum$viewport2[i]<-as.numeric(unlist(strsplit(myviewport,'x'))[2])
}
allsum$excludeviewport<-0
w<-which(allsum$viewport1<1000)
allsum$excludeviewport[w]<-1
w<-which(allsum$viewport2<550)
allsum$excludeviewport[w]<-1
w<-which(allsum$viewport1>2500)
allsum$excludeviewport[w]<-1

t.test(abs(allsum$RDT.LI)~allsum$excludeviewport)
t.test(abs(allsum$WC.LI)~allsum$excludeviewport)
...
dopsum$ftcdOK<-1
w<-which(is.na(dopsum$A1)) #these are cases with no signal
dopsum$ftcdOK[w]<-0
dopsumorig$ftcdOK<-dopsum$ftcdOK
