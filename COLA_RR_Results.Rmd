...
var1<-bivdat$tempx[bivdat$Group==1]
var2<-bivdat$tempy[bivdat$Group==1]
cor1<-spearman.ci(var1, var2, nrep = 1000, conf.level = 0.95)
var1<-bivdat$tempx[bivdat$Group==2]
var2<-bivdat$tempy[bivdat$Group==2]
cor2<-spearman.ci(var1, var2, nrep = 1000, conf.level = 0.95)
...
lab1<- paste0("Group 1: rho = ",round(cor1$estimate,3),": 95% CI: ",round(cor1$conf.int[1],3),' to ',round(cor1$conf.int[2],3))
lab2<- paste0("Group 2: rho = ",round(cor2$estimate,3),": 95% CI: ",round(cor2$conf.int[1],3),' to ',round(cor2$conf.int[2],3))
...
myplot <- ggplot(bivdat, aes(x=tempx, y=tempy, color=Handedness)) +
  ...
  geom_hline(yintercept=0,linetype="dashed")+
   geom_vline(xintercept=0,linetype="dashed")+
  annotate("text", x=-5, y=9.5, label= lab1,size=2.8) +
  annotate("text", x=-5, y=8, label= lab2,size=2.8)   
  return(myplot)
}
...
The total battery consisted of the following tests (described in more detail in a later section:    
...
![Timings within a single trial for the six tasks used with fTCD](tasktimings.png) 
...
![Participant recruitment flowchart](COLAflow.jpg)
...
ggsave(paste0(mydir,"/",figdir,"/allLRbeh.png"),width = 6, height = 12)
#also save to current project to enable access when knitting to word
ggsave("allLRbeh.png",width = 6, height = 12)
fignumber<-fignumber+1

```

![Responses to stimuli on left and right: online behavioural laterality tasks_](allLRbeh.png)
...
pirateplot(data = longdata.b, LIz ~ Handed * Task,  cex.lab=1.5, cex.axis=1, cex.names=1.4,ylab='R hem bias <--- LIz ---> L hem bias')
...
ggsave(paste0(mydir,"/",figdir,"allbiv.png"),width = 4, height = 12)
ggsave('allbiv.png',width = 4, height = 12)
fignumber<-fignumber+1
```
...
```{r maketimecourse,echo=F,message=F}
...
#NB. I can't get this to use same y-axis scale for all 6 figures - once you add the 2ndary axis it defaults to fitting to largest number. 
#Will need fiddling - I can thing of 2 ways of making it behave - a) add spurious large value , maybe if one is added that is outside xaxis range, this will affect y axis but not plot?
## Otherwise, only solution seems to be to hand-craft the 2nd axis.
## The yaxis scaling is fine provided you omit the 2ndary axis. 
...
`r fignumber <- fignumber+1`
![Timecourse of left and right hemisphere blood flow velocity (left axis) and L-R difference (right axis) for six tasks in left- and right-handers)](alltimecourse.png)
...
![One-factor vs. two-factor structural equation models](bothF1F2.tiff)  
...
semPaths(fit1, "diagram", weighted = FALSE,  shapeMan = "rectangle", sizeMan = 12, 
    sizeMan2 = 5,edge.color='black',nodeLabels=mynodes, filetype='tiff',filename=pathfigname1,width=3,height=1.8) #draws a path diagram
...
semPaths(fit2, "diagram", weighted = FALSE,  shapeMan = "rectangle", sizeMan = 12, 
    sizeMan2 = 5,edge.color='black',nodeLabels=mynodes2,filetype='tiff',filename=pathfigname2,width=3,height=1.8) #draws a path diagram
...
imageX<-image_read(paste0(pathfigname1,'.tiff'))
imageY<-image_read(paste0(pathfigname2,'.tiff'))
itemname<- paste0(alldir,'/bothF1F2.tiff')
itemnamej<-paste0(alldir,'/bothF1F2.jpg')
...
cropY <- image_crop(imageY, "800x0+60")
  ...
  imgboth <- image_append(imgboth)
  imgboth<-image_trim(imgboth)
 
  tname <- "~/Dropbox/COLA_RR_Analysis/03-graphic-outputs/bothF1F2.tiff"
  jname <- "~/Dropbox/COLA_RR_Analysis/03-graphic-outputs/bothF1F2.jpg"
...
semPaths(fit.2Fn, "diagram", weighted = FALSE,  shapeMan = "rectangle", sizeMan = 12, 
    sizeMan2 = 5,nodeLabels=mynodes3,filetype='tiff',filename=pathfigname,width=3,height=1.8) #draws a path diagram
...
![Heatmap with behavioural and fTCD laterality indices](bigheatmap.png)  
...
Sentence Decision also behaved unexpectedly, in that it had significant loadings on Factor 1, despite being designed as a purely receptive task. While it is possible that participants might have rehearsed sentences to themselves when doing the task, the fast pace of the task made that unlikely, and furthermore, our previous study suggested that simple production of spoken output was not the key attribute of Factor 1 tasks; we and others have found that repeating overlearned sequences such as numbers or days of the week is not lateralised (Dhanjal et al., 2008;  Woodhead, Rutherford & Bishop, 2020).  The Sentence Decision task did, however, involve  using syntactic information to assign semantic roles and build meaning representations, and hence more linguistic computation than the tasks that used single word stimuli. 
