...
The .Rdata files are available on OSF - they are much smaller than .csv so easier to work with.

Before running this script, need to download __bothsess_dat.rds__ (https://osf.io/5twkh/).

For testing the script, we have made a folder on the desktop called 'testscript', containing a folder called 'COLAbits'. The former acts our working directory and the latter contains all files that are read or written to by the script. You should set up your paths to fit this structure. The names can be changed if you wish. 

You need to start by copying these files to COLAbits:  
  bothsess_dat.rds  
  coded_demog.csv  
  grammarScoring.csv  
  grammarScoringshort.csv  
  
The latter two files are found in https://osf.io/25jmz/.  

N.B. This file generates a few tables and plots as sanity checks, but these are not used in the main analysis.  
...
In addition to calculating LIs for all tasks, this script calculates split half LIs from odd/even items, and test-retest reliability for the subset of people who did session 3 as well as session 1.
...
```{r read}
# get the data
# Currently set up to read from Rdata files.
# This is much quicker than reading original .csv files, but still long enough for you to go away and make a cup of coffee while it happens

mydir <- "~/Desktop/testscript"  #working directory
datdir<-paste0(mydir,"/COLAbits/") #folder in working directory that contains all ancillary files that are read from or written to
...
n1<-nrow(coded_demog)
nall<-nrow(allsum)
coded_demog[(n1+1):nall,]<-NA #blank rows for session 3
...
```{r lexTALE}
    ...
    # mark those with lexTALE < 60
    allsum <- allsum %>% dplyr::mutate(lexExclude= if_else(lexTALE < 60, 1, 0),
...
While there are a number of items, only 95 of these 'critical items' in the scoring portion of the supplemental materials. The scoring treats each response option as a separate item - see "grammarScoring.csv" which is taken from Supplementary materials of Hartshorne et al. 

(N.B. Jan 2022: QUERIES ABOUT SOME ITEMS - NOTHING DONE ABOUT THESE EXCEPT WHERE TYPOS INVOLVED. Correspondence with Hartshorne did not elicit any response).  
...
This showed that more than one error on short form could be used. Here we compute both long and short form errors. In practice, we don't use these - see Registered Report. 
...
```{r Grammar}
   ...
   colnames(pic.dat)<-c('subject','score')
    ...
    grammar.score <- read.csv(paste0(datdir,"grammarScoring.csv"))
    grammar.score12 <- read.csv(paste0(datdir,"grammarScoringshort.csv")) #short version
...
A generic function that can be used for different tasks to make a laterality index and write it to the allsum file.
...
```{r headphonecheck,echo=F}
h1<-all_dat[all_dat$Task.Name=="headphone check",]
h1<-h1[h1$ANSWER %in% c('First','Second','Third'),]
h1$head1correct <-0
w<-which(h1$ANSWER==h1$Response)
h1$head1correct[w]<-1
h.agg<-aggregate(h1$head1correct,by=list(h1$subject),FUN=sum)

h2<-all_dat[all_dat$Task.Name=="headphone check (2)",]
h2<-h2[h2$ANSWER %in% c('First','Second','Third'),]
h2$head2correct <-0
w<-which(h2$ANSWER==h2$Response)
h2$head2correct[w]<-1
h.agg2<-aggregate(h2$head2correct,by=list(h2$subject),FUN=sum)
...
for (i in 1:nrow(allsum)){
    subj <- allsum$subject[i]
    w<-which(h.agg$Group.1==subj)
    if(length(w)>0)
     {allsum$head1corr[i]<-h.agg$x[w]}
      w<-which(h.agg2$Group.1==subj)
    if(length(w)>0){
     allsum$head2corr[i]<-h.agg2$x[w]
    }
}
allsum$headfail<-0
w<-which(allsum$head1corr<4)
allsum$headfail[w]<-1
w<-which(allsum$head2corr<4)
allsum$headfail[w]<-1
  
#table(allsum$headfail) -not run, confirms all those included here passed headphone check
...
Here we process the chimeric faces and colour scales. These are specific to the Bangor team and are not included in the Registered Report.  
...
#This chunk redone line by line to ensure we do have the file correctly aligned.
allsum<-read.csv(paste0(datdir,"allsum.csv"))

combsum <-allsum[allsum$session==1,]
dopsum <- read.csv("~/Dropbox/COLA_RR_Analysis/02-data/02.2_ftcd/ftcd_data.csv")
dopsumorig<- read.csv("~/Dropbox/COLA_RR_Analysis/02-data/02.2_ftcd/ftcd_data_origbaseline.csv") #create alternative combined dataset with original baseline for supplement
colnames(dopsum)[1]<-'IDdop'

nucols<-colnames(dopsum)
combsum[,nucols]<-NA
for (i in 1:nrow(combsum)){
  w<-which(dopsum$ID==combsum$ID[i])
  if(length(w)>0){
    combsum[i,nucols]<-dopsum[w,nucols]
  }
    
}

colnames(dopsumorig)[1]<-'IDdop'
combsumo<-combsum 
nucols<-colnames(dopsumorig)
combsumo[,nucols]<-NA
for (i in 1:nrow(combsumo)){
  w<-which(dopsumorig$ID==combsumo$ID[i])
  if(length(w)>0){
    combsumo[i,nucols]<-dopsumorig[w,nucols]
  }
    
}

#original merge command - gives same result
...
for (f in 1:2){
  if(f==2)
    {combsum<-combsumo} #original baseline version
...
if(f==2){
writedir<-paste0(mydir,"/02-data/combined_data_origbaseline.csv")
}
