...
modellist<-c('f1 =~ A_P1+B_P2+C_P3+D_R1+E_R2+F_R3',
             'f1 =~ A_P1+B_P2+C_P3, f2 =~ NA*D_R1+E_R2+F_R3, f2~~1f2',
             'f1 =~ A_P1+B_P2+C_P3 +E_R2, f2 =~ NA*F_R3+C_P3+D_R1+E_R2, f2~~1f2')
...
Exploratory analysis used a random half of the sample, and took mean LIs for odd and even trials to ensure model identification. We started with the bifactor model of Woodhead et al. (2021) and dropped nonsignificant paths. 
The model syntax was specified in lavaan:
...
Summary output with the initial test sample was as follows:

```{r oddevenout,echo=F}
summary(fit.2FZWa.1)
myfitmeasures<-c('CFI','TLI','SRMR','rmsea','chisq','DF') 
fitmeasures(fit.2FZWa.1,myfitmeasures)

```

For the holdout sample, the summary was:

```{r oddevenout2,echo=F}
summary(fit.2FZWa.2)
fitmeasures(fit.2FZWa.2,myfitmeasures)
