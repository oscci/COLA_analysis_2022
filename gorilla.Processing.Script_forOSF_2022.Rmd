...
nsub <-length(allsub) 
allsum <- data.frame(matrix(ncol = 16, nrow = nsub))  # create data frame to write to
...
colnames(allsum) <- 
  c("subject", "session","ID","age", "male", "Rfooted", "Rhanded", "education", "bilingual", "languages", "AoA.English",
    "mother.LH", "father.LH", "siblings", "siblings.LH", "centre") #centre is testing location
...
RTprocess<-function(file.in,allsum,colprefix){
  ...
  meanRT <- aggregate(data= allcorr, FUN=mean, na.rm=T,RT~ subject+Response)
  sdRT <- aggregate(data= allcorr, FUN=sd, na.rm=T,RT~ subject+Response)
  NRT <- aggregate(data= allcorr, FUN=length, RT~ subject+Response)

  for (i in 1:nrow(allsum)){
    mysub <- allsum$subject[i]
     allsum$pcorr[i]<-accdata$Correct[accdata$subject==mysub]
    allsum$Lmean[i]<-meanRT$RT[meanRT$Response=='left' & meanRT$subject==mysub]
    allsum$Lsd[i]<-sdRT$RT[sdRT$Response=='left' & sdRT$subject==mysub]
    allsum$LN[i]<-NRT$RT[NRT$Response=='left' & NRT$subject==mysub]
    allsum$Rmean[i]<-meanRT$RT[meanRT$Response=='right' & meanRT$subject==mysub]
    allsum$Rsd[i]<-sdRT$RT[sdRT$Response=='right' & sdRT$subject==mysub]
    allsum$RN[i]<-NRT$RT[NRT$Response=='right' & NRT$subject==mysub]
  }
...
ncases <- nrow(DL.aggL)
allsum$DL.R.odd<-allsum$DL.R.even<-allsum$DL.L.odd<-allsum$DL.L.even<-allsum$DL.R <- allsum$DL.L <- NA #initialise columns
for(i in 1:ncases){  #doing row by row to ensure properly aligned
  ...
  allsum$DL.R.odd[w] <- DL.aggR.odd[i,2]
   allsum$DL.L.odd[w] <- DL.aggL.odd[i,2]
  allsum$DL.R.even[w] <- DL.aggR.even[i,2]
   allsum$DL.L.even[w] <- DL.aggL.even[i,2]
...
allsum$DL.LI.odd<-round(100*(allsum$DL.R.odd-allsum$DL.L.odd)/(allsum$DL.R.odd+allsum$DL.L.odd),1)
allsum$DL.LI.even<-round(100*(allsum$DL.R.even-allsum$DL.L.even)/(allsum$DL.R.even+allsum$DL.L.even),1)
...
mycor=cor.test(myfile$DL.LI.odd,myfile$DL.LI.even)
mycor
plot(myfile$DL.LI.odd,myfile$DL.LI.even,col=(1+myfile$Rhanded))
...
for (i in 1:3){
  ...
  if (i==1){
  cR <- which(colnames(allsum)=='DL.R.odd')
  cL <- which(colnames(allsum)=='DL.L.odd')
  }
    if (i==2){
  cR <- which(colnames(allsum)=='DL.R.even')
  cL <- which(colnames(allsum)=='DL.L.even')
  }
...
if (i==1){
  colnames(allsum)[nc:(nc+1)]<-c('DL.n.odd','DL.zlat.odd')
}
if (i==2){
  colnames(allsum)[nc:(nc+1)]<-c('DL.n.even','DL.zlat.even')
}
...
combsum$DL.zlat.e<-combsum$DL.zlat
combsum$DL.zlat.e[combsum$excludeDL==1]<-NA
combsum$RDT.zlat.e<-combsum$RDT.zlat
combsum$RDT.zlat.e[combsum$excludeRDT==1]<-NA
combsum$WC.zlat.e<-combsum$WC.zlat
combsum$WC.zlat.e[combsum$excludeWC==1]<-NA

combsum$A.LI.e<-combsum$A_mean_LI
combsum$A.LI.e[combsum$A_exclude==1]<-NA
combsum$B.LI.e<-combsum$B_mean_LI
combsum$B.LI.e[combsum$B_exclude==1]<-NA
combsum$C.LI.e<-combsum$C_mean_LI
combsum$C.LI.e[combsum$C_exclude==1]<-NA
combsum$D.LI.e<-combsum$D_mean_LI
combsum$D.LI.e[combsum$D_exclude==1]<-NA
combsum$E.LI.e<-combsum$E_mean_LI
combsum$E.LI.e[combsum$E_exclude==1]<-NA
combsum$F.LI.e<-combsum$F_mean_LI
combsum$F.LI.e[combsum$F_exclude==1]<-NA
...
table1(~ DL.zlat.e+RDT.zlat.e+WC.zlat.e+A.LI.e+B.LI.e+C.LI.e+D.LI.e+E.LI.e+F.LI.e|factor(langexclude) , data=ftcdyes)
...
aovALIe <- aov(A.LI.e ~ langexclude, data = ftcdyes)
# Summary of the analysis
summary(aovALIe)

aovBLIe <- aov(B.LI.e ~ langexclude, data = ftcdyes)
# Summary of the analysis
summary(aovBLIe)

aovCLIe <- aov(C.LI.e ~ langexclude, data = ftcdyes)
# Summary of the analysis
summary(aovCLIe)

aovDLIe <- aov(D.LI.e ~ langexclude, data = ftcdyes)
# Summary of the analysis
summary(aovDLIe)

aovELIe <- aov(E.LI.e ~ langexclude, data = ftcdyes)
# Summary of the analysis
summary(aovELIe)

aovFLIe <- aov(F.LI.e ~ langexclude, data = ftcdyes)
# Summary of the analysis
summary(aovFLIe)
