...
mysummary[3,writecolnum]<-paste0(round(semTools::skew(thisdat$thiscol)[1],2),' (',pformat(semTools::skew(thisdat$thiscol)[4]),')')
mysummary[4,writecolnum]<-paste0(round(semTools::kurtosis(thisdat$thiscol)[1],2),' (',pformat(semTools::kurtosis(thisdat$thiscol)[4]),')')
...
mysummary[11,writecolnum]<-paste0(round(cor.eo$estimate,2),' [',round(cor.eo$conf.int[1],2),', ',round(cor.eo$conf.int[2],2),']')
...
for (m in 1:6){
  ...
  thisstring <- paste0(round(ss$est[myrow],2)," [",round(ss$se[myrow],2),"]")
...
if(length(myrow)>0){
  thisstring <- paste0(round(ss$est[myrow],2)," [",round(ss$se[myrow],2),"]")
  bigsummary[14,writecol]<-thisstring
}

wantfits <- c('CFI','TLI','SRMR','rmsea','chisq','df','rmsea.ci.lower','rmsea.ci.upper') #fit indices to include
...
bigsummary <- data.frame(matrix(NA,nrow=21,ncol=4))
colnames(bigsummary)<-c('Estimate','Model.1F','Model.2F','Model.2Fn')
bigsummary[,1]<-c('Standardized paths [SE]','A -> Fac1','B -> Fac1','C -> Fac1','D -> Fac1','E -> Fac1','F -> Fac1','A -> Fac2','B -> Fac2','C -> Fac2','D -> Fac2','E -> Fac2 ','F -> Fac2','Fac1~~Fac2','Fit indices','CFI','TLI','SRMR','RMSEA [95% CI]','chisq','DF')
```
