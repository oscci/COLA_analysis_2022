...
supplementary <- 1 #set this to zero for the main paper. Here we change the selection criteria to look at implications of using original language exclusion cutoff
...
![Figure `r fignumber`: Participant recruitment flowchart: preregistered (planned) and actual numbers](participant_flow.png)
 
```{r makeflowchart,echo=F}
#DiagrammeR is difficult to knit into Word - only seems to work with html. So I am using a separate R script that will create the file in the Viewer from which it can be saved to png and then read in here. The file to create this is makeparticipantflow. It will only work if testtab has been populated by this script. Output currently needs to be saved manually.
makeparticipant <-0
if(makeparticipant==1){
