...
ss<- summary(myfit,standardized=TRUE)$PE
 srow<-nrow(ss)
 scol<-ncol(ss)
...
Analyses that exclude participants on the basis of preregistered cutoffs on language test scores are provided in Supplementary materials. Differences in means, variances and covariances for the full sample and the restricted sample are all very small and there are no meaningful differences to results from statistical tests or modeling. 
...
#These cases to be checked; For time being we exclude them when computing means, but will need to update this after checking.
datalist<-combdat[-poordata,c(1,acccols,ncol(combdat)-1,ncol(combdat))]
...
THIS CURRENTLY EXCLUDES 8 CASES WITH V LOW SCORES.  
...
Following recommendations by Schermelleh-Engel et al. (2003) we report here values for $\chi$^2^, CFI, TLI, RMSEA and SRMR.
...
In a further exploratory analysis, we compared left- and right-handers on mean factor scores. For Factor 1, the effect size for handedness (Cohen's d) was `r es1`, and a t-test gave t = `r t1`, df = `r df1`, `r pf1`. For Factor 2, the effect size for handedness (Cohen's d) was `r es2`, and a t-test gave t = `r t2`, df = `r df2`, `r pf2`. 
...
## Additional analyses  
It seemed possible that the laterality indices might be influenced by either the language status of participants, and/or behavioural scores on the fTCD tasks (number of items generated or percentage correct). 
...
colnames(pairframe)<-c('LI','Performance','N','rho','p-value','rho:Games with Words','p-value','rho: Lextale','p-value')
pairframe[,1]<-LInames
pairframe[,2]<-perfnames

for (n in 1:ntest){
  mycor <- cor.test(combdat[,LIs[n]],combdat[,perf[n]],method='spearman')
 
  mycorx <-cor.test(combdat[,LIs[n]],combdat[,perf[n]])#pearson just so we get df for N!
    mycorg <- cor.test(combdat[,LIs[n]],combdat$gramerrs,method='spearman')
    mycorl <-cor.test(combdat[,LIs[n]],combdat$lexTALE,method='spearman')
...
I THINK IT WOULD BE WORTH NOTING HERE PRIOR WORK ON DICHOTIC vs. FMRI - CHECK WITH DAVID/EMMA?
...
CAN WE LINK FACTORS TO THIS ANALYSIS? https://pubmed.ncbi.nlm.nih.gov/35267079/
...
## Full results from Structural Equation models, comparing output with different subsets of participants.  
...
```{r suppoutputs,echo=F}
bigsumfit1 <- initbigsummary()
bigsumfit1$X<-NA #extra column

colnames(bigsumfit1)[2:4]<-c('Fit1: Full Sample','Fit1: Selected Sample','Fit1: Full Sample - Orig. baseline','Fit1: Selected Sample - Orig. baseline')
...
rdat <- filter(ddati,langgroup<3)
rdatOB <-filter(ddatOB,langgroup<3)
...
colnames(bigsumfit2n)[2:5]<-c('Modified 2 Factor: Full Sample','Modified 2 Factor: Selected Sample','Modified 2 Factor: Full Sample - Orig. baseline','Modified 2 Factor: Selected Sample - Orig. baseline')
```
...
Summary output from the single factor model can be compared for the preregistered criteria (Selected sample with original baseline from -10 to 0 s, last column), and the modified criteria used in the main analysis (Full sample with baseline from -5 to 2 s, second column).  

### Covariance and means matrix 
```{r covmatrix,echo=F}
...
```{r formatbigsummaryx,echo=F} 
#couldn't get this to work as function!
for (c in 2:ncol(bigsumfit1)){
w<-which(bigsumfit1[,c]=='NA [NA]')
bigsumfit1[w,c]<-'-'
}
ftb<-flextable(bigsumfit1)
...
The detailed fit statistics for the single factor model, with the final criteria (Full sample with baseline -5 to 2s) are shown below.

### Unstandardized path estimates  

```{r printestimates}
ft_pe <- parameterEstimates(fit1A)
...
```{r printsestimates}
ft_se <- standardizedsolution(fit1A)
ft_se[,4:9]<-numformat(ft_se[,4:9],3)
flextable(ft_se)
```
 

### Model-implied fitted covariance matrix    
```{r printfitted}
ft_fit <- as.data.frame(fitted(fit1A))
ft_fit <- numformat(ft_fit,3)
ft_fit <-add_rownames(ft_fit)
flextable(ft_fit)
```
 

### Unstandardized residuals of fitted model     
```{r printresids}
ft_res <- as.data.frame(residuals(fit1A))
ft_res[2:ncol(ft_res)] <- numformat(ft_res[2:ncol(ft_res)],3)
ft_res<-add_rownames(ft_res)
flextable(ft_res)
```
