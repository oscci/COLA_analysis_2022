...
measurementInvariance(model=model.2Fn,data=ddati,meanstructure=T,group="Handed")
...
model.2Fp <- '
f1 =~  NA*A_P1 + B_P2+C_P3+E_R2  #4 loadings per factor
...
measurementInvariance(model=model.2Fp,data=ddati,meanstructure=T,group="Handed")
...
This is just exploratory - I've set oddeven to 0 at start of chunks so it does not automatically run. Just preserving it here in case we want to revisit this idea - which seemed sensible at the time....
...
```{r sandpit}
#Checking out computation of chi square
obs<-cov(ddati[,4:9]) #observed covariances
obs1 <- obs[!upper.tri(obs)] #all variances and covariances as vector
#residuals are obs-expected, so let's try for fit 1
res<-resid(fit1)$cov
res1<-res[!upper.tri(res)]
...
for(i in 1:6){
  for (j in i:6){
    v<-v+1
    rc[v]<-paste0(rr[i],'.',cc[j])
}
}
df<-as.data.frame(cbind(rc,obs1,res1))
df$exp<-df$obs1+df$res1

exp<-obs1+res1

chi<-res1^2/exp

#Goal is to work out how chisq is computed, but what I have found is abnormally large variance of E_R2.  This seems to be introduced by normalisation!
...
```{r sandpit2}
#check how bifactor model behaves - basically it fails to converge
#https://gist.github.com/rpietro/7968980


#Can get a reasonable result if I just have factor 1 and general factor - but this is getting more similar to the earlier model : though it does uncorrelated factors
#
bifactor <- '
general.factor  =~ A_P1 + B_P2+C_P3 + D_R1 + E_R2
factor1         =~ A_P1 + B_P2+C_P3
#factor2         =~ F_R3+ D_R1 + E_R2
...
'
ddat1<-ddati[ddati$randgroup==1,] #group1 only for exploring
fitbiv.1 <- cfa(bifactor, data=ddat1,meanstructure=T)
tabbiv1<-makeSEMtab(fitbiv.1)

measurementInvariance(model=bifactor,data=ddati,group="randgroup")
