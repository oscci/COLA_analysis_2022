...
^8^School of Psychological Sciences, University of Western Australia 
...
## Procedure
There were two stages to the project. All participants who proceeded beyond the initial screening  completed the online testing, and a subset of participants were invited back for the second session (fTCD). Participants received course credit or were paid in accordance with guidelines at their local testing centre (at least £8 per hour).  
...
## Online behavioural battery: Distribution of Laterality indices  
...
Previously, Woodhead et al. (2021) had found weaker covariance between factors in left-handers than in right-handers. To test whether this was the case for the current dataset, we added a further constraint, which was that the covariance between factors should be the same for the two groups. The covariance between factors was numerically lower in left-handers (`r myests[1]`, SE = `r myests[2]`) than right-handers (`r myests[3]`, SE = `r myests[4]`), but the difference was not great, and model fit was not impaired when the covariance was forced to be identical in the two handedness groups. 
...
In a further exploratory analysis, we compared left- and right-handers on mean factor scores: values for Factor 1 were mean =  `r numformat(fact1$estimate[1],2)`, SD = `r numformat(agfac1$x[1],2)`; for Factor 2, mean =  `r numformat(fact1$estimate[2],2)`, SD = `r numformat(agfac1$x[2],2)`.  For Factor 1, the effect size for handedness (Cohen's d) was `r es1`, and a t-test gave t = `r t1`, df = `r df1`, `r pf1`. For Factor 2, the effect size for handedness (Cohen's d) was `r es2`, and a t-test gave t = `r t2`, df = `r df2`, `r pf2`. 
...
## Relationship among fTCD and behavioural laterality indices
